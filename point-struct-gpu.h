#ifndef POINT_STRUCT_GPU_H
#define POINT_STRUCT_GPU_H
...
struct PointStructGPU : public PointStruct<T>
{
	...
	__forceinline__ __host__ __device__ static int compareDim1(PointStructGPU<T> &pt1, PointStructGPU<T> &pt2)
		{return pt1.compareDim1(pt2);};
	__forceinline__ __host__ __device__ int compareDim1(PointStructGPU<T> &other) const
	{
	...
	__forceinline__ __host__ __device__ static int compareDim2(PointStructGPU<T> &pt1, PointStructGPU<T> &pt2)
		{return pt1.compareDim2(pt2);};
	__forceinline__ __host__ __device__ int compareDim2(PointStructGPU<T> &other) const
	{
	...
	__forceinline__ __host__ __device__ int comparisonTiebreaker(PointStructGPU<T> &other) const
	{
		return this == &other ? 0 : this < &other ? -1 : 1;
	};

	__forceinline__ __host__ __device__ bool operator==(PointStructGPU<T> &other) const
	{
...
template <typename T>
std::ostream &operator<<(std::ostream &os, const PointStructGPU<T> &ptstr)
{
	ptstr.print(os);
	return os;
}
