...
$(foreach suffix_type,$(suffixes), \
	$(subst .c,.d,%.$(suffix_type)): %.$(suffix_type); $(create_prereq))
...
# In each iteration over the list .c .cpp .cu, suffix_type takes on the value of the list that is active in the current iteration, then finds the words in sources matching the pattern <stem>.<suffix_type> and substitutes that suffix with .d
include $(foreach suffix_type,$(suffixes), \
	$($(subst .c,.d,$(filter %.$(suffix_type),sources))))
endif
