...
{
	// Report all nodes in left subtree, continue left search in right subtree
	...
	if (range_split_poss)
	{
				...
				&& GPUTreeNode::hasRightChild(curr_node_bitcode))
		{
			...
			else
			{
				...
				if (left_child_dim2_val >= min_dim2_val)
				{
					// Use coalesced threads to allocate space, as not all threads are guaranteed to need to run this code for reporting left leaf children
					...
					{
						res_arr_d[res_ind_to_access].dim1_val = StaticPSTGPUArr<T, PointStructTemplate, IDType, num_IDs>::getDim1ValsRoot(tree_root_d, tree_num_elem_slots)[left_child_search_ind];
						res_arr_d[res_ind_to_access].dim2_val = left_child_dim2_val;
						if constexpr (HasID<PointStructTemplate<T, IDType, num_IDs>>::value)
															...
															)
{
	if (GPUTreeNode::hasLeftChild(curr_node_bitcode)
			&& GPUTreeNode::hasRightChild(curr_node_bitcode))
	{
		// Delegate reporting of all nodes in right child to another thread if one is available; otherwise, process that node here
		if (threadIdx.x + target_thread_offset < blockDim.x)
		{
